...
@SpringBootTest
public class OptimisticLectureInsertServiceTest extends IntegrationTest {
    ...
    void 낙관적_락을_이용한_특강_신청_인원_제한_동시성_테스트() throws Exception {
        ...
        for (int i = 1; i <= userCount; i++) {
            ...
            executorService.execute(() -> { // 각 사용자에 대해 스레드 실행
                try {
                    lectureRegistrationService.apply(lecture.getId(), userId); // 고유한 userId로 특강 신청 시도
    ...
    void 동일_유저가_같은_특강을_5번_신청했을_때_1번만_성공하는지_검증() throws Exception {
        ...
        for (int i = 0; i < requestCount; i++) {
            executorService.execute(() -> {
                try {
                    lectureRegistrationService.apply(lectureSchedule.getId(), userId);
